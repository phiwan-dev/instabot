...
class Flux:
    ...
    def get_image(self, prompt: str):
        if self.pipe is None:
            self.pipe = FluxPipeline.from_pretrained(self.save_path, torch_dtype=torch.float16)
...
if __name__ == "__main__":
    ...
    flux.get_image(prompt="a bedroom with a fireplace")
    flux.get_image(prompt="a kitchen with a fireplace")
